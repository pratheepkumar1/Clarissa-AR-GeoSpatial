...
public class GeospatialManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // MY CODE
        VpsAvailabilityPromise vpsPromise = AREarthManager.CheckVpsAvailabilityAsync(pose.Latitude, pose.Longitude);

        StartCoroutine(checkVpsAvailabilityPromise(vpsPromise));
    ...
    private IEnumerator checkVpsAvailabilityPromise(VpsAvailabilityPromise promise)
    {
        ...
        {
            VpsResults = promise.Result.ToString();
            yield break;
        }          
        else
            VpsResults = "Pending";
